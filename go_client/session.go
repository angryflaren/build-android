...
func RunSession(
	...
	if err != nil {
		return false, fmt.Errorf(
			"генерация сертификата: %w",
			err,
		)
	}

	dtlsCfg, err := dtlsFactory.Build(cert)
	if err != nil {
		return false, fmt.Errorf(
			"создание DTLS конфигурации: %w",
			err,
		)
	}
	...
	log.Printf(
		"[ВОРКЕР #%d] [DTLS] Рукопожатие (Handshake)...",
		sessionID,
	)
	...
	hcancel()
	<-handshakeSem // release immediately after handshake
	...
	if err != nil {
		if useWrap {
			...
			if strings.Contains(errStr, "deadline") ||
				...
				return false, fmt.Errorf(
					"WRAP_AUTH_TIMEOUT: DTLS timeout, пароль/WRAP не подтверждён",
				)
			}
		}
	...
	log.Printf(
		"[ВОРКЕР #%d] [DTLS] Соединение установлено ✓",
		sessionID,
	)
