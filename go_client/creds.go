...
func getTokenChain(
	...
	client, err := tlsclient.NewHttpClient(
		...
		tlsclient.WithClientProfile(browserProfile.TLSProfile),
		tlsclient.WithRandomTLSExtensionOrder(),
		tlsclient.WithCookieJar(jar),
	)
	if err != nil {
		return "", "", nil,
			fmt.Errorf("failed to initialize tls_client: %w", err)
	}
	...
	log.Printf(
		"[STREAM %d] [VK Auth] Identity - Name: %s | UA: %s",
		streamID,
		name,
		browserProfile.Profile.UserAgent,
	)
	...
	for attempt := 0; ; attempt++ {
		...
		if errObj, hasErr := resp["error"].(map[string]interface{}); hasErr {
			...
			if captchaErr != nil && captchaErr.RedirectURI != "" && captchaErr.SessionToken != "" {
				...
				successToken, solveErr := solveCaptchaBySelectedMode(ctx, streamID, attempt+1, captchaErr, client, browserProfile.Profile, savedProfile)
				if solveErr != nil {
