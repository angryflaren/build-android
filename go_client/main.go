...
func main() {
	...
	log.Printf("[КЛИЕНТ] Воркеров: %d (групп: %d, максимум в группе: %d)", *numW, numGroups, workersPerGroup)
	...
	disp := NewDispatcher(ctx, localConn, stats, nil)
	defer disp.Shutdown()
	...
	for g := 0; g < numGroups; g++ {
		...
		remaining := *numW - (workerIDCounter - 1)
		if remaining <= 0 {
			break
		}
		groupWorkers := workersPerGroup
		if remaining < groupWorkers {
			groupWorkers = remaining
		}
		
		ids := make([]int, groupWorkers)
